...
void main() {
  const scanner = ChangelogScanner(_kChangelog);
  group('standard', () {
    for (var entry in _kExpected.entries) {
      test('standard changelog detection: ${entry.key}', () {
        expect(
          scanner.getChanges(entry.key),
          entry.value.trim(),
        );
      });
    }
  ...
  group('subversion', () {
    for (var entry in _kMinorExpected.entries) {
      test('subversion changelog detection: ${entry.key}', () {
        expect(
          scanner.getChanges(
            entry.key,
          ),
